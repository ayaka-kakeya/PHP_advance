            ...
            <div class="row justify-content-center">
                <div class="col-10">   
                    <table class="table table-bordered">
                            ...
                            {section name=row loop=$list}<tr><td>{$list[row].name}</td><td>{$list[row].kana}</td><td>{$list[row].tel}</td><td>{$list[row].email}</td><td style="white-space:pre-wrap">{$list[row].body}</td><td><a href="/contact/getupdate?id={$list[row].id}">編集</a></td><td><a href="/contact/delete?id={$list[row].id}" onclick="return confirm('本当に削除しますか？')">削除</a></td></tr>{/section}
